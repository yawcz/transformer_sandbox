...
def run_swiglu(
    ...
    model.w1.load_state_dict(
        {
            "weight": w1_weight,
        }
    )
    model.w2.load_state_dict(
        {
            "weight": w2_weight,
        }
    )
    model.w3.load_state_dict(
        {
            "weight": w3_weight,
        }
    )
...
def run_multihead_self_attention(
    ...
    multihead_self_attention.q_proj.load_state_dict(
        {
            "weight": q_proj_weight,
        }
    )
    multihead_self_attention.k_proj.load_state_dict(
        {
            "weight": k_proj_weight,
        }
    )
    multihead_self_attention.v_proj.load_state_dict(
        {
            "weight": v_proj_weight,
        }
    )
    multihead_self_attention.output_proj.load_state_dict(
        {
...
def run_multihead_self_attention_with_rope(
    ...
    Returns:
        ...
        implementation with the given QKV projection weights and input features.
    """
    multihead_self_attention_with_rope = MultiheadSelfAttention(d_model, num_heads, max_seq_len, theta)
    multihead_self_attention_with_rope.q_proj.load_state_dict(
        {
            "weight": q_proj_weight,
        }
    )
    multihead_self_attention_with_rope.k_proj.load_state_dict(
        {
            "weight": k_proj_weight,
        }
    )
    multihead_self_attention_with_rope.v_proj.load_state_dict(
        {
            "weight": v_proj_weight,
        }
    )
    multihead_self_attention_with_rope.output_proj.load_state_dict(
        {
            "weight": o_proj_weight,
        }
    )
...
def run_train_bpe(
    ...
    **kwargs,
    ...
    freq = build_freq_dict_from_file(input_path, special_tokens, os.cpu_count() or 4)
    return do_merges(freq, vocab_size, special_tokens)
